...
int conY;
int pot1;
int joystick_button; // 1= pot used for speed, 0 = joy stick sole controller of speed
int dcpin;
int foward;
int backward;
int dirf;
...
void loop()
{
  ...
  else if (radio.available())
  {
    ...
    Serial.print( final_package[0]);
    Serial.print("\t" + String(final_package[1]));
    Serial.print("\t" + String(final_package[2]));
    Serial.println("\t" + String(final_package[3]));
...
void ReverseFunc(int seconds)
{
      ...
      reverse_command[0] = 255;
      reverse_command[1] = 127; //neutral x position 
...
void SetMotors()
{
  ...
  {
    Speed = map(pot1, 0, 1023, 255, 0);
    dirf = 0;
    dirr = 1;
  }
  else if (conY >= 140)
  {
    Speed = map(pot1, 0, 1023, 0, 255);
    dirf = 1;
    dirr = 0;
  }

  // Servo
  if ((conX > 110) && (conX < 140))
  {
    spos = 90;
  }
  else if (conX <= 110)
  {
    spos = map(conX, 0, 110, 50, 90);
  }
  else
  {
    spos = map(conX, 140, 255, 90, 120);
  }
}

void Act_DC()
{
  digitalWrite(foward, dirf);
  digitalWrite(backward, dirr);
...
void SetMotorsNoPot()
{
  ...
  {
    Speed = map(conY, 0, 1023, 255, 0);
    dirf = 0;
    dirr = 1;
  }
  else if (conY >= 140)
  {
    Speed = map(conY, 0, 1023, 0, 255);
    dirf = 1;
    dirr = 0;
  }
